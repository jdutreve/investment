...
SYSTEM_THRESHOLDS: dict[str, float] = {
    ...
    # Corpus/embedding (M7). `embedding_dims` is meant to be the CONTRACT
    # between stored vectors and the live model — compared against
    # `InProcessEmbedder.dims` so swapping EMBEDDING_MODEL for a different-
    # dimension variant fails loudly instead of writing vectors that can never
    # be compared to the existing ones. That check has NO HOST YET (there is no
    # main.py/startup path before M8), so all three are seeded and UNWIRED, per
    # this table's convention: the value is pinned now so the ingester and the
    # eventual startup assertion read one source, not three literals.
