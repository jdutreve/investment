...
- **drawdown** — the stack's realized 36M rolling drawdown against
...
- **market-data freshness** — the newest MarketData row vs today. This is the
  one that actually protects capital. The stack's only downside control is the
  200d overlay, which re-reads its moving average once per monthly decision; if
  the data pipeline dies, the overlay goes blind while the code keeps reporting
  a perfectly normal month. In 2020 the fall took 25 days.
...
async def stack_drawdown_alert(db: InvestmentDB) -> Alert | None:
    """The -25% rule, on the stack's realized 36M rolling drawdown.
    ...
    return Alert(
        ...
        code="stack_drawdown",
        message=(
            f"The stack is {drawdown * 100:.1f}% below its 36-month high, past your "
            f"{limit * 100:.0f}% rule (as of {rows[0]['ts']}). Nothing has been blocked — the "
            "200d overlay remains the mechanism that acts. This is for your decision."
        ),
    )
