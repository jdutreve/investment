...
exists for: the weekly chain against a manual run, the cron against the
...
async def test_a_second_holder_is_refused_and_told_what_is_running() -> None:
    ...
    assert raised.value.holder == "weekly-chain"
    assert "weekly-chain" in str(raised.value)
...
async def test_the_lock_is_not_re_entrant() -> None:
    """Deliberate: the weekly chain runs UC8 as a step, so a re-entrant lock
...
async def test_a_concurrent_task_is_refused_rather_than_queued() -> None:
    ...
    assert await second() == "refused: weekly-chain"
    release.set()
